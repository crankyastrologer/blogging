...
def get_art(id):
    x = postdbs.query.filter_by(owner=id)
    a = 0
    for i in x:
        a += 1
        return a
...
def get_foll(id):
    ...
    a = 0
    if x.followers is not None:
        list1 = x.followers.split(" ")
...
def check_following(id):
    ...
    if x.following is not None:
        list1 = []
        list1 = x.following.split(' ')
        for i in list1:
...
def follow(id):
    if user_login:
        x = User.query.filter_by(email=email).first()
        if x.following is not None:
            x.following = x.following + f"{id} "
        else:
            x.following = f'{id} '

        y = User.query.filter_by(id=id).first()
        if y.followers is not None:
            y.followers = y.followers + f'{x.id} '
        else:
            y.followers = f'{x.id}'
        db.session.commit()
        return redirect('/')
...
def my_profile():
    if user_login:
        x = User.query.filter_by(email=email).first()
        id = x.id
        return render_template('my_profile.html', users=x.name, user=user_login,article = get_art(id),followers = get_foll(id))
    else:
        redirect_uri = url_for('login', _external=True)
        return redirect(redirect_uri)
...
def profile(id):
    ...
    if person.email == email:
        ...
        return render_template('profile.html', person=person, user=user_login, following=following, article=get_art(id),
                               followers=get_foll(id))
